...
static void sendData(){
  printf("sending data\n");
  collectData();
  uip_udp_packet_sendto(sender_conn, &data, sizeof(data),
...
PROCESS_THREAD(sender_process, ev, data){
  ...
  uip_ip6addr(&receiver_ipaddr, UIP_DS6_DEFAULT_PREFIX, 0, 0, 0, 0, 0, 0, 1);

  /* setup udp connection */
  sender_conn = udp_new(NULL, UIP_HTONS(UDP_RECEIVER_PORT), NULL);
  if(sender_conn == NULL) {
  ...
  printf("Created a connection with the receiver\n");
